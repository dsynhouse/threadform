...
/** Exhausting the shared command budget ends the whole plan; a fault confined
 * to one object does not. Only this class stops the object loop. */
class CommandBudgetError extends Error {}
...
export function generatePlan(project: Project): StitchPlan {
  ...
  for (const raw of project.objects) {
    // One unusable object reports itself and the rest of the design still
    // generates. Its partial commands are discarded and the emitter state is
    // restored, so a fault cannot leak into the next object's routing.
    const objectStart = stitches.length,
      blockStart = blocks.length,
      lastBefore = last,
      threadBefore = previousThread;
    try {
      ...
      if (previousThread && previousThread !== threadKey)
        push(last, o, "color");
            ...
            for (const point of route.slice(1))
              sew(
                ...
                Math.min(
                  o.length,
                  3,
                  underlay ? activeUnderlayLength : Infinity,
                ),
                underlay,
              );
            return;
      ...
      if (o.sequinMode) {
        ...
        // Underlay is generated separately by underlayPrograms; this emits
        // cover stitches only.
        const fill = (extraAngle = 0, layer = 0) => {
          const a = ((o.angle + extraAngle) * Math.PI) / 180;
          const paths = contours
            .filter((p) => p.length >= 3)
            .map((path) => path.map((p) => rotate(p, -a)));
          const satin = ["satin", "raised-satin"].includes(o.type);
          const b = bounds(paths),
            spacing = Math.max(0.2, o.spacing) / (satin ? 2 : 1);
          const sewRow = (
            ...
            const max = satin ? satinMaxLength : Math.max(0.6, o.length);
            if (satin && length > satinMaxLength) splits = true;
            if (o.type === "program-split") {
          ...
          };
          {
            const bands = tatamiBands(
              ...
              o.fillRule,
              spacing,
              o.type !== "tatami" ? undefined : o.spacingEnd,
      ...
      stitches.length = objectStart;
      blocks.length = blockStart;
      last = lastBefore;
      previousThread = threadBefore;
      activeUnderlayLayer = undefined;
      const message =
        error instanceof Error ? error.message : "Stitch generation failed.";
      issues.push({
        level: "error",
        objectId: raw.id,
        message: message.startsWith(raw.name)
          ? message
          : `${raw.name}: ${message}`,
      });
      // The budget is shared, so continuing would only repeat the failure.
      if (error instanceof CommandBudgetError) break;
    }
  }
