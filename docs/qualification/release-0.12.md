...
| Workspace dragging | Selecting an object on pointer-down reset the active gesture. A newly allocated selection array could cancel dragging even with the same selected IDs. | Compare selection values and preserve the selection change owned by the active drag. Commit the actual release position, keep locked/hidden objects stationary, and avoid creating Undo for a click. Chrome checks cover selected and unselected objects, lock protection, Undo, and save/reload of the moved position. |
| Type checks after both builds | Next and Vinext wrote incompatible route declarations to the same generated type directory. | The type-check command regenerates Next route declarations before running TypeScript and propagates either command's failure. It works after the Sites build as well as before it. |
...
The final drag check reproduced the reported failure before the fix: a pointer drag left the column at X −5.06 mm, Y 47.01 mm and did not create an Undo entry. After the fix the same selected-object drag moved it to X −12.98 mm, Y 41.77 mm; Undo restored the original coordinates. A first-gesture drag with no selection then moved it to X 0.71 mm, Y 42.98 mm. A locked-object drag left those coordinates unchanged. After unlocking, saving and reloading, revision 30 retained that position and the original 23.75 × 13.30 mm dimensions.
