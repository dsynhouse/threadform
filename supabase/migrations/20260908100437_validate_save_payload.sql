...
begin
  ...
  if p_id is null or p_save_id is null or p_expected is null or p_expected<0 or p_expected>=2147483647 then raise exception 'Invalid revision' using errcode='22023';end if;
  if p_project is null or jsonb_typeof(p_project)<>'object' or p_project->'version' is distinct from '1'::jsonb or jsonb_typeof(p_project->'name') is distinct from 'string'
  ...
  if v_count<0 or v_name is null or length(v_name)<1 or length(v_name)>100 then raise exception 'Invalid project' using errcode='22023';end if;
  ...
  if found then
    if v_retry.project_id<>p_id or v_retry.project<>p_project then raise exception 'Save identifier reused' using errcode='23505';end if;
...
create or replace function public.threadform_schema_version() returns integer language sql immutable set search_path='' as $$select 4$$;
